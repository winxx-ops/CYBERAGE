...
# Curated high-impact path dictionary targeting sensitive files, administrative portals, and APIs
TARGET_PATHS = [
...
class ContentDiscoveryEngine:
    ...
    def run_discovery(self) -> Tuple[List[DiscoveredPath], List[Finding]]:
        """
        Executes concurrent HTTP HEAD/GET probes across common paths.
        Returns discovered paths and any security findings generated.
        """
        ...
        # Sort by status code (200 first, then redirects, then 403)
        discovered.sort(key=lambda x: (x.status_code != 200, x.status_code))

        # Evaluate discovered paths for sensitive information leaks
    ...
    def _probe_path(self, path: str, desc: str) -> Optional[DiscoveredPath]:
        ...
        try:
            # First try polite HEAD request
            resp = self.session.head(target_url, timeout=3.0, allow_redirects=False, verify=False)

            # Some servers block HEAD or return 405 Method Not Allowed; fall back to GET with small byte limit
            ...
            # Record if endpoint exists (200, redirects, or explicit 401/403 access control triggers)
            ...
            if item.status_code == 200:
                if ".git" in item.path:
                    ...
                    findings.append(Finding(
                        ...
                        recommendation="Remove '.env' files from the web root and configure server rewrite rules to deny access to dotfiles.",
                        confidence=Confidence.HIGH,
                    ))
                    ...
                    findings.append(Finding(
                        ...
                        recommendation="Store backup files outside the public web root directory.",
                        confidence=Confidence.HIGH,
                    ))
                    ...
                    findings.append(Finding(
                        ...
                        recommendation="Ensure private APIs and schema documentation require authentication in production.",
                        confidence=Confidence.HIGH,
                    ))
